...
*
     ...
     -          GLR(MXGL,0:NBLD,NTBL), GLC(MXGL,0:NBLD,NTBL)    ! SCR, SCC
      ...
      DIMENSION FL(5,3)
      DIMENSION AM(3,9)
      INTEGER   MCNTL(30)   ! 「CNTL」カードのデータ内容(XMQ配列に相当)
      COMMON /ETC/MCNTL
...
*
      ...
      READ(QD(12:53),'(7F6.0)') W1,W2,(X(I),I=153,156),X(158)
      X(150)=SIN(DR*W1)
      X(151)=COS(DR*W1)
      X(152)=(W2-135.)/15.
      X(154)=0.01*X(154)
...
*
  160 CONTINUE
      CALL DCHECK(QD,759,NERR)
*
      READ(QD(12:71),'(20I3)') (M(I),I=171,190)
      N=0
  161 M1=171+2*N
      IF(N.LE.10.AND.M(M1).NE.0) THEN
...
*
      ICYCL=1
  502 CALL INWD(NUW,MCNTL(5)/2,ICYCL,WDND,IDND,ISTAT)   ! 気象データの読み込み
...
*
      ICYCLO=ICYCL
      CALL INWD(NUW,MCNTL(5)/2,ICYCL,WDND,IDND,ISTAT)
...
*
      SG=UG*VG
      X(L+J+39)=GF(SS**2)*SG/X(L+25)
  529 IF(X(L+24).EQ.0.) THEN  ! 壁面全体面積-窓面積=0                           ! rev 20121213(T.Nagai) Debug
       X(L+J+23)=0.                                                             ! add 20121213(T.Nagai) Debug
      ELSE                                                                      ! add 20121213(T.Nagai) Debug
       X(L+J+23)=SS*(ST-SG)/X(L+24)                                             ! rev 20121213(T.Nagai) Debug
...
*
  620 CONTINUE
      ...
      END IF
*
      EXC=WD(1,J)-X(155)+W*(X(LE+J+23)*X(L+9)+SH(J)*X(L+10))
...
***        'BUIL' CHECK LIST ********************
     ...
     *     -7,   15,-90.0,90.0,   15,-180.0,180.0/
      DATA M(567),M(568),M(569),M(570),M(571),X(572),X(573)/
     *     14,   9,0,100,   13,-50.0,100.0/
      DATA (M(I),I=574,578)/   9,0,100,   12,   16/
*
***        'CNTL' CHECK LIST ********************
      DATA (M(I),I=1001,1047)/ -7,   1,0,1,   1,0,1,   1,0,1,   1,0,1,
     *   1,0,2,   1,0,99,   1,1,12,   1,1,31,   1,0,99,   1,1,12,
     *   1,1,31,   1,0,99,   1,1,12,   1,1,31,   1,1,999, 16/
*
***        'HRAT' CHECK LIST ********************
...
*
***        'SDAY' CHECK LIST ********************
      DATA (M(I),I=759,820)/   -7,   3,1,12,   3,1,31,   1,1,12,
     *      1,1,31,   1,1,12,   1,1,31,   1,1,12,   1,1,31,   1,1,12,
     *      1,1,31,   1,1,12,   1,1,31,   1,1,12,   1,1,31,   1,1,12,
     *      1,1,31,   1,1,12,   1,1,31,   1,1,12,   1,1,31,   16/
*
***        'OSCH' CHECK LIST ********************
...
*     REQ. ROUTINES       - NDATE
*********************************************************************
      SUBROUTINE INWD(NUW,IOPWE,ICYCL,WD,ID,ISTAT)
      DIMENSION WD(7,24),ID(7,5)
*
  100 DO 200 I=1,7
  200 READ(NUW,'(24F3.0,3I2,2I1)',END=999)
